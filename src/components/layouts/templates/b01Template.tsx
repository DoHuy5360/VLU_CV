	...
	return (
			<div className='flex flex-col relative bg-white'>
				<div className='flex items-center justify-center sticky top-0 bg-white border-b-[1px]'>
					<a href="#P-Contact" className='p-2 cursor-pointer'>Contacts</a>
					<a href="#P-About" className='p-2 cursor-pointer'>About</a>
					<a href="#P-Skill" className='p-2 cursor-pointer'>Skills</a>
					<a href="#P-Experience" className='p-2 cursor-pointer'>Experiences</a>
					<a href="#P-Projects" className='p-2 cursor-pointer'>Projects</a>
				</div>
				<div className='flex flex-col p-32 gap-24' style={{scrollBehavior: "smooth"}}>
					...
					<div id="P-Skill" className='flex flex-col items-center gap-3'>
						...
						<div className='flex gap-2'>
							{
								data.skills.map(e =>{
									return <div key={e.id} className='flex flex-col gap-2 items-center'>
					...
					<div id="P-Experience" className='flex flex-col items-center gap-3'>
						...
						<div className='flex flex-col gap-3'>
							{data.experiences.map(e =>{
								return <div key={e.id} className='grid grid-cols-3 rounded-lg border-[1px] p-4 overflow-hidden'>
									<div className="">
										{
											e.images.map(i => <Image key={i.id} src={i.src} width={100} height={100} className="outline outline-4 outline-slate-200" alt={i.label} />
											)
										}
									</div>
					...
					<div id="P-Projects" className='flex flex-col items-center gap-3'>
						...
						<div className='flex flex-col gap-3'>
							{
								data.projects.map(e =>{
									return <div key={e.id} className='grid grid-cols-2 border-[1px] rounded-lg overflow-hidden'>
												<div>
													{e.images.map(i => <Image key={e.id} src={i.src || '/image/user.jpg'} width={400} height={400} className='w-auto h-auto' alt={i.label} />)}
												</div>
											...
											<div className='flex gap-1'>
											{
												e.technologies.map(t => 
													...
													<div key={t.id} className='rounded-lg text-xs p-1 bg-slate-200'>{t.name}</div>
												)
											}
										</div>
									</div>
