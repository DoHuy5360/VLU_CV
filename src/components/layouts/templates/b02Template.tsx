				...
				<div id='P-Experience' className='flex flex-col items-center gap-3'>
					...
					<div className='flex flex-col gap-3'>
						{data.experiences.map((e) => {
							return (
								<div key={e.id} className='grid grid-cols-3 gap-10 rounded-lg border-[1px] p-4 overflow-hidden'>
									<div>
										{e.images.map((i) => (
											<Image key={i.id} src={i.src || "/image/user.jpg"} width={100} height={100} className='outline outline-4 outline-slate-200' alt={i.label} />
										))}
									</div>
